...
(function () {
  ...
  const TOPBAR_SUPABASE_URL = 'https://eliwawvkkcamntciekhd.supabase.co';
  const TOPBAR_SUPABASE_KEY = 'sb_publishable_HLgA595j21f-RVPwR8zGFg_8LjGJnq0';
...
`;
  ...
  async function pushWaterMergedToSupabase(localWater) {
    // Only do this when we're NOT on the water page itself — that page
    // owns its own localStorage state directly and doesn't need the
    // topbar to also push on its behalf.
    if (window.location.pathname.endsWith('/po-water.html') ||
    ...
    try {
      ...
      const { data } = await supa
        .from('app_state').select('data').eq('key', 'po-water').maybeSingle();
      const current = (data && data.data) || {};
      const merged = Object.assign({}, current, { po_water_v1: localWater });
      await supa.from('app_state').upsert(
        { key: 'po-water', data: merged, updated_at: new Date().toISOString() },
        { onConflict: 'key' }
      );
    } catch (e) { /* offline — local change will sync next time user visits the water page */ }
  }
